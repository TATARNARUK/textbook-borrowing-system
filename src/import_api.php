...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    function saveBook($pdo, $data) {
        // Map ข้อมูล
        $title = $data['name'] ?? '-';
        $isbn  = (!empty($data['isbn']) && $data['isbn'] != '-') ? $data['isbn'] : $data['code'];
        $author = $data['author'] ?? '-';
        $price = $data['price'] ?? 0;
        
        // 🔥 จัดการรูปภาพ (แก้ URL ให้ถูกต้อง)
        $cover = $data['image'] ?? '';
        $cover = str_replace(' ', '%20', $cover); // แก้ช่องว่างใน URL
        ...
        // เช็คซ้ำ
        $stmtCheck = $pdo->prepare("SELECT COUNT(*) FROM book_masters WHERE isbn = ?");
    ...
    if (isset($_POST['import_book'])) {
        ...
        if(saveBook($pdo, $bookData)){
            echo "<script>setTimeout(function() { Swal.fire('สำเร็จ!', 'นำเข้าเรียบร้อย', 'success'); }, 500);</script>";
        } else {
            echo "<script>setTimeout(function() { Swal.fire('แจ้งเตือน', 'หนังสือเล่มนี้มีอยู่แล้ว', 'warning'); }, 500);</script>";
        }
    }
    ...
    if (isset($_POST['import_all'])) {
        ...
        echo "<script>setTimeout(function() { Swal.fire('เสร็จสิ้น!', 'นำเข้าหนังสือใหม่จำนวน $count เล่ม เรียบร้อย', 'success'); }, 500);</script>";
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .card-book { transition: transform 0.2s; border: none; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        .card-book:hover { transform: translateY(-5px); }
        .book-cover { height: 220px; object-fit: cover; border-radius: 10px 10px 0 0; }
        .btn-import-all { background: linear-gradient(45deg, #198754, #20c997); color: white; border: none; font-weight: bold; }
        .btn-import-all:hover { box-shadow: 0 5px 15px rgba(25, 135, 84, 0.4); transform: translateY(-2px); color: white; }
    </style>
</head>
<body>
    ...
    <div class="container py-5">
        <div class="d-flex justify-content-between align-items-center mb-4 bg-white p-4 rounded-4 shadow-sm">
            ...
            <div class="d-flex gap-2">
                <form method="POST" onsubmit="return confirm('ยืนยันที่จะนำเข้าหนังสือทั้งหมด?\n(หนังสือที่มีอยู่แล้วจะถูกข้าม)');">
                    <button type="submit" name="import_all" class="btn btn-import-all rounded-pill px-4">
                        <i class="fa-solid fa-layer-group me-2"></i> นำเข้าทั้งหมด
                    </button>
                </form>
        ...
        <?php if (!empty($books)): ?>
            <div class="row g-4">
                <?php foreach ($books as $book): 
                    ...
                    $b_img = str_replace(' ', '%20', $b_img_raw); // แก้ช่องว่าง
                    
                    // ถ้ามาแค่ชื่อไฟล์ ให้เติม Path (ตาม JSON ที่คุณส่งมา)
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mt-auto">
                                <form method="POST">
                                    ...
                                    <input type="hidden" name="cover" value="<?php echo htmlspecialchars($b_img); ?>"> <input type="hidden" name="pdf" value="<?php echo htmlspecialchars($b_pdf); ?>">
                                    ...
                                    $chk->execute([$b_isbn]);
                                    if ($chk->fetch()) {
                                        echo '<button type="button" class="btn btn-secondary w-100 btn-sm rounded-pill" disabled><i class="fa-solid fa-check"></i> นำเข้าแล้ว</button>';
                                    } else {
